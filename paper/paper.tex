...
\begin{proposition}[Series Recognition Soundness]
...
\begin{definition}[The $\sthat$–ring fragment]\label{def:st-fragment}
  Let $\mathcal{L}_{\sthat}$ be the set of closed terms generated from real constants using
...
\begin{proof}[Proof sketch]
  In our implementation, the evaluation of all constructors in $\mathcal{L}_{\sthat}$ does not consult the ultrafilter, and $\sthat$ is computed by a local Laurent‑series recognizer (returning $\st(x)$ when it succeeds). Therefore the result depends only on the underlying sequences, not on membership choices made by $\PUF$. See \code{Hyperreal.standard\_part} (which calls \code{is\_near\_standard\_by\_series}) for the exact pathway.%
\end{proof}
