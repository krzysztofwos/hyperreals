...
% =====================================================================
\section{Standard Part Extraction via Series Recognition and Closure Rules}\label{sec:standard}

The (classical) standard part map $\st$ connects finite hyperreal computations back to real analysis. Our runtime exposes a \emph{partial} extractor $\sthat$ that succeeds only when it can certify, in a completion-invariant decidable fragment, that a hyperreal is near-standard with that standard part; otherwise it returns failure. The core mechanism is Laurent-series recognition in $\delta = 1/n$ for analytic terms, but we additionally implement conservative closure rules that certify bounded--vanishing tails (bounded $\cdot$ infinitesimal) and selected non-analytic limits at infinity (e.g., $\sthat(\tanh(1/\delta))=1$ and $\sthat(\exp(-1/\delta))=0$).
...
The series recognizer has inherent limitations. It cannot handle expressions with top-level oscillatory terms like $\sin(n)$ or $(-1)^n$, as these do not admit series expansions. It also cannot handle general division where the denominator has multiple terms, as this would require computing infinite series inversions. Nevertheless, the overall extractor $\sthat$ can sometimes certify near-standardness in the presence of oscillatory factors when they appear only in vanishing tails; for example, $\sthat(\sin(n)/n)=0$ and $\sthat(1+(-1)^n/n)=1$ follow from the closure rule ``bounded $\cdot$ infinitesimal is infinitesimal.'' We also include selected non-analytic but completion-invariant limits at infinity such as $\sthat(\tanh(n))=1$ and $\sthat(\exp(-n))=0$ (where $n=1/\delta$).
...
Despite these limitations, the series recognizer successfully handles a significant fragment of hyperreal arithmetic, including all polynomial expressions in $n$ and $1/n$, and compositions of analytic functions applied to near-zero arguments; the extended extractor additionally covers bounded--vanishing tails and the small limit library described above.
...
\subsection{Bounded--Vanishing Closure and Selected Non-analytic Limits}\label{sec:sthat-closure}

While the Laurent-series recognizer covers a large analytic fragment, near-standardness can also be certified in some non-analytic yet completion-invariant cases. Concretely, our implementation extends $\sthat$ with conservative closure rules that (i) certify that bounded oscillatory factors become negligible when multiplied by an infinitesimal, and (ii) recognize a small library of standard limits at $\pm\infty$ (e.g., $\tanh(x)\to\pm 1$ and $\exp(x)\to 0$ as $x\to-\infty$). These rules never consult the partial ultrafilter; they rely only on cofinite reasoning about $n$ and uniform bounds such as $|\sin(x)|\le 1$.
