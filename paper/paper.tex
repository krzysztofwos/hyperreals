...
\begin{abstract}
  Hyperreal numbers, which extend the reals with infinitesimals and infinite quantities, provide elegant foundations for analysis but are built from non-constructive objects (non-principal ultrafilters) whose existence depends on the Axiom of Choice. We present a practical implementation of hyperreal arithmetic through a lazy partial ultrafilter construction that commits only to constraints demanded by program execution. Comparisons are resolved by deterministic finite-or-cofinite and certified asymptotic reasoning (including Laurent-series recognition and monotone limit propagation) when possible, and otherwise by incremental SAT solving. We also implement partial standard part extraction on a completion-invariant fragment: beyond Laurent-series recognition in $\delta = 1/n$ for analytic terms, we certify bounded--vanishing tails, bounded-over-diverging ratios, and selected monotone limits at infinity. Finally, we expose an explicit completion-selecting standard-part approximator based on SAT-guided bisection, enabling controlled exploration of choice-dependent finite hyperreals.
\end{abstract}
...
Our implementation realizes this theoretical framework through careful engineering. We maintain a partial ultrafilter as a conjunction of constraints, employing SAT solving for consistency checking when extending it. However, we identify a substantial decidable fragment where comparisons can be resolved without SAT solving, including finite-or-cofinite reasoning and conservative asymptotic facts (uniform bounds and monotone limit propagation) beyond the primitive cases involving standard reals, the infinitesimal $1/n$, and the infinite hyperreal $n$. For standard part extraction, we combine Laurent-series recognition in $\delta = 1/n$ with a conservative asymptotic-facts analyzer (bounded--vanishing tails, bounded-over-diverging ratios, and selected monotone limits at infinity). For inherently choice-dependent finite hyperreals, we additionally expose an explicit completion-selecting standard-part approximator based on SAT-guided bisection.
...
\paragraph{Asymptotic-Facts-Determined Order}
Beyond series-determined cases, we also employ a conservative asymptotic-facts analyzer that propagates uniform bounds and selected monotone limits at $\pm\infty$. When it can certify that a difference $b_n-a_n$ is eventually positive or negative (or that a defining predicate holds cofinally), the corresponding comparison set is classified as cofinite (or finite) without SAT. This captures comparisons involving $\exp(n)$ and $\log(1+n)$ in regimes where a Laurent expansion in $\delta$ is unavailable, and supports fast-path reasoning for bounded-over-diverging ratios.
...
% =====================================================================
\section{Standard Part Extraction via Certified Asymptotic Facts}\label{sec:standard}

The (classical) standard part map $\st$ connects finite hyperreal computations back to real analysis. Our runtime exposes a \emph{partial} extractor $\sthat$ that succeeds only when it can certify, in a completion-invariant decidable fragment, that a hyperreal is near-standard with that standard part; otherwise it returns failure. The implementation combines (i) Laurent-series recognition in $\delta = 1/n$ for analytic terms and small arguments with (ii) a conservative asymptotic-facts analyzer that propagates uniform bounds and monotone limits at $\pm\infty$ and certifies bounded--vanishing tails, bounded-over-diverging ratios, and selected growth-dominance lemmas (e.g., $\sthat(\tanh(n))=1$, $\sthat(\exp(-n))=0$, $\sthat(\sin(n)/\log(1+n))=0$, and $\sthat(n/\exp(n))=0$).

Completion-dependent ultralimits of top-level oscillatory sequences (e.g., $\sin(n)$ or $(-1)^n$) remain outside this completion-invariant fragment, and $\sthat$ returns failure on such inputs.
...
Division requires special care. Within the Laurent-series recognizer we support division only when the denominator is a monomial (a single power of $\delta$). This restriction ensures that the series engine does not introduce infinite inversions that our finite truncation cannot represent. The overall extractor can nevertheless certify additional division patterns via asymptotic facts, for example bounded-over-diverging ratios (standard part $0$) and quotients of near-standard values with nonzero standard part.
...
The completion-invariant extractor $\sthat$ is intentionally incomplete. It returns a real number only when it can certify, using series recognition and conservative asymptotic facts, that the underlying representative sequence has an ordinary real limit. Top-level oscillatory sequences such as $\sin(n)$ and $(-1)^n$ are finite but admit completion-dependent ultralimits; accordingly $\sthat$ returns failure on such terms.

Beyond the analytic Laurent/Taylor fragment, our asymptotic-facts layer proves near-standardness in several additional completion-invariant situations:
\begin{itemize}[itemsep=2pt,topsep=4pt]
\item \textbf{Bounded--vanishing tails}: if $r(n)$ is uniformly bounded and $g(n)\to 0$ (e.g.\ $g(n)=1/n$), then $r(n)g(n)\to 0$; thus $\sthat(\sin(n)/n)=0$ and $\sthat(1+(-1)^n/n)=1$.
\item \textbf{Monotone limits at infinity}: if $x_n\to\pm\infty$, then $\tanh(x_n)\to\pm 1$ and $\exp(x_n)\to 0$ for $x_n\to-\infty$. This composes with monotone divergence propagation (e.g.\ $\log(1+n)\to\infty$), yielding $\sthat(\tanh(\log(1+n)))=1$ and $\sthat(\exp(-\log(1+n)))=0$.
\item \textbf{Bounded-over-diverging ratios}: if $|a_n|$ is bounded and $|b_n|\to\infty$, then $a_n/b_n\to 0$, so $\sthat(\sin(n)/\log(1+n))=0$.
\item \textbf{Growth dominance}: a small lemma library proves additional completion-invariant limits that do not admit Laurent expansions, including polynomial ratio limits and super-polynomial decay. For example, $\sthat((n^2+1)/(n^2+n))=1$, $\sthat(\log(1+n)/n)=0$, and $\sthat(n^k\exp(-n))=0$ for fixed $k$ (in particular $\sthat(n/\exp(n))=0$).
\end{itemize}

These extensions enlarge the completion-invariant fragment while preserving soundness: each successful case reduces to a cofinite convergence argument in $\R$.

\subsection{Completion-dependent Standard Part Selection (Optional)}\label{sec:stchoose}

For many finite hyperreals, the classical standard part $\st(x)$ exists in each completed ultraproduct model but is \emph{not} completion-invariant as a function of the representative sequence. In such cases, returning a unique real number necessarily amounts to selecting a completion. To support controlled exploration of this choice dependence, our implementation provides an explicit completion-dependent routine \code{choose\_standard\_part} that approximates a standard part value by SAT-guided interval bisection: it repeatedly queries comparisons of the form $x < q$ for rational midpoints $q$, uses feasibility checks to determine whether each branch is forced or underdetermined, and commits a truth value when a choice is available. The returned approximation is therefore indexed by the resulting completion path (and by the tie-breaking policy) and is intentionally outside the completion-invariant fragment of Theorem~\ref{thm:obs-eq-st}.
...
When a comparison is requested, the partial ultrafilter first checks whether the result is determined by finite-or-cofinite or certified asymptotic analysis. Sets like $\{n : n < 10\}$ (finite) are immediately excluded, while sets like $\{n : n > 10\}$ (cofinite) are immediately included. This fast path avoids SAT solving for many common comparisons.
...
The asymptotic analysis subsystem supports both standard part extraction and fast-path reasoning about comparisons. It combines a conservative asymptotic-facts analyzer (bounds, monotone limits at infinity, and bounded-over-diverging ratios) with a Laurent-series engine for analytic expansions in $\delta = 1/n$. The series engine represents expansions as dictionaries mapping integer exponents to real coefficients, supporting sparse representations efficiently.
...
The correctness of our implementation rests on three pillars: the soundness of the partial ultrafilter construction, the validity of the asymptotic analysis subsystem (series recognition plus conservative asymptotic facts), and the consistency of the overall system. We establish each through a combination of theoretical analysis and empirical validation.
...
The series recognizer is the core analytic component of $\sthat$; it computes standard parts through Laurent series expansion. Its correctness depends on the mathematical validity of the series manipulations and the proper handling of convergence. The next subsection covers additional conservative closure rules used by the implementation.
...
\subsection{Certified Asymptotic Facts for $\sthat$}\label{sec:sthat-closure}

While the Laurent-series recognizer covers a large analytic fragment, near-standardness can also be certified in some non-analytic yet completion-invariant cases using conservative asymptotic facts. Concretely, our implementation extends $\sthat$ with rules that (i) certify bounded--vanishing tails (bounded $\cdot$ infinitesimal), (ii) certify bounded-over-diverging ratios, and (iii) propagate selected monotone limits at $\pm\infty$ through a small library of functions (e.g., $\log(1+x)\to\infty$ for $x\to\infty$, $\tanh(x)\to\pm 1$, and $\exp(x)\to 0$ for $x\to-\infty$). These rules never consult the partial ultrafilter; they rely only on cofinite reasoning about $n$ and uniform bounds such as $|\sin(x)|\le 1$.

\begin{proposition}[Extended $\sthat$ Soundness]\label{prop:sthat-sound}
  If the standard-part extractor $\sthat$ returns a real number $c$ for a hyperreal $h$, then $h - c$ is infinitesimal in any completion of the partial ultrafilter.
\end{proposition}

\begin{proof}
  The extractor can succeed in several ways.

  \paragraph{(i) Series recognition.}
  If $\sthat(h)=c$ is obtained by the Laurent-series recognizer, the claim follows immediately from Proposition~\ref{prop:series-sound}.

  \paragraph{(ii) Infinitesimal remainders (bounded--vanishing).}
  The extractor may also return $c=0$ after certifying that $h$ is infinitesimal using conservative algebraic rules, including the closure principle ``bounded $\cdot$ infinitesimal is infinitesimal.'' These rules are justified by standard real bounds (e.g., $|\sin(x)|\le 1$, $|\cos(x)|\le 1$, $|\tanh(x)|\le 1$) together with the fact that $\delta=1/n\to 0$. Hence $h_n\to 0$ in the ordinary real sense. For any $\varepsilon>0$, the set $A_\varepsilon=\{n:|h_n|<\varepsilon\}$ is cofinite, and therefore lies in every non-principal completion, which is exactly the statement that $h-0$ is infinitesimal.

  \paragraph{(iii) Certified limits at infinity.}
  The extractor includes a small library of completion-invariant limits at $\pm\infty$, together with conservative rules to certify that an argument diverges (either by inspecting a dominant Laurent term in $\delta$ or by monotone divergence propagation through functions such as $\log(1+x)$). For example, if it returns $\sthat(\tanh(g))=1$ (resp.\ $-1$), it has certified that $g_n\to +\infty$ (resp.\ $g_n\to -\infty$). By the classical real limits $\lim_{x\to+\infty}\tanh(x)=1$ and $\lim_{x\to-\infty}\tanh(x)=-1$, for any $\varepsilon>0$ there exists $M$ such that $|\tanh(x)-1|<\varepsilon$ whenever $x\ge M$. Since $g_n\to+\infty$, there exists $N$ such that $g_n\ge M$ for all $n\ge N$, hence $\{n:|\tanh(g_n)-1|<\varepsilon\}$ is cofinite. The same argument applies to $\sthat(\exp(g))=0$ when $g_n\to-\infty$ using $\lim_{x\to-\infty}\exp(x)=0$.

\paragraph{(iv) Bounded-over-diverging ratios.}
  The extractor may also return $c=0$ after certifying that a quotient $h=a/b$ satisfies a uniform bound $|a_n|\le M$ and divergence $|b_n|\to\infty$. Then $|h_n| = |a_n|/|b_n| \le M/|b_n|\to 0$, so for every $\varepsilon>0$ the set $\{n:|h_n|<\varepsilon\}$ is cofinite, hence $h-0$ is infinitesimal in every completion.

\paragraph{(v) Arithmetic closure.}
  The extractor is closed under $+,-,\cdot,/$ on already-certified standard parts. If $x-a$ and $y-b$ are infinitesimal, then $(x\pm y)-(a\pm b)$ and $(xy-ab)$ are infinitesimal by closure of infinitesimals under addition and multiplication. For division, if additionally $b\ne 0$ and $y_n\to b$, then $y$ is eventually bounded away from $0$, so $x/y$ is near-standard with standard part $a/b$.

  In all cases, for every $\varepsilon>0$ the set $\{n:|h_n-c|<\varepsilon\}$ is cofinite and therefore belongs to every non-principal completion, proving that $h-c$ is infinitesimal in any completion.
\end{proof}


\subsection{System Consistency}

The overall system maintains consistency through careful interaction between components. When the partial ultrafilter makes a choice, it affects all future comparisons involving related expressions. The standard-part extractor $\sthat$ (series recognition plus conservative asymptotic facts) operates independently of these choices for values it can certify, ensuring that extracted standard parts are well-defined regardless of ultrafilter completion.
...
\begin{proof}
  ...
  Whenever evaluation reaches an occurrence $\sthat(h)$, the extractor either fails, in which case $\sthat(h)=\bot$ under every completion, or it returns a real $c$.
  In the success case, Proposition~\ref{prop:sthat-sound} implies that $h-c$ is infinitesimal in \emph{every} completion, hence $\st(h)=c$ in both $\PUF$ and $\PUF'$.
...
We evaluate our implementation across three dimensions: the effectiveness of fast paths in avoiding SAT solving, the growth rate of the partial ultrafilter, and the coverage of the completion-invariant standard-part extractor $\sthat$ (series recognition plus conservative asymptotic facts). Our results demonstrate that the lazy approach is not only theoretically sound but also practically efficient.
...
\IfFileExists{metrics.tex}{\input{metrics}}{%
  \begin{table}[t]
    \centering
    \caption{Evaluation metrics table omitted from this snapshot.}
    \label{tab:metrics}
  \end{table}
}
...
Our fast paths successfully avoid SAT solving for a meaningful fraction of comparisons. Across our benchmark suite (40 membership queries), 19 (47.5\%) are resolved without SAT via finite-or-cofinite classification, conservative asymptotic reasoning, and memoization. The remaining comparisons predominantly involve oscillatory sequences such as $\sin(n+k)$ that necessarily defer to choice-dependent reasoning.
...
\subsection{Standard Part Coverage}

The completion-invariant extractor $\sthat$ successfully computes standard parts for all near-standard expressions in our benchmarks. When series recognition applies, it returns the constant term of a Laurent expansion in $\delta = 1/n$; otherwise, conservative asymptotic facts certify additional cases such as bounded-over-diverging ratios and monotone limits at infinity. The truncation at order 10 provides sufficient precision for our test cases, achieving errors between $10^{-9}$ and $10^{-12}$ for standard part extraction when a numeric approximation is required.

The series component's performance is predictable: arithmetic operations have linear complexity in the series order, while function compositions have quadratic complexity due to series multiplication. For our default order of 10, all operations complete in microseconds on modern hardware.
...
Several researchers have explored nonstandard models in program semantics and hybrid systems. Benveniste et al.~\cite{Benveniste2012} propose a non-standard semantics of hybrid-system modelers, while Hasuo and Suenaga~\cite{HasuoSuenaga2012} apply nonstandard analysis to static analysis of hybrid systems. These works use nonstandard analysis as a mathematical tool but do not implement computational infinitesimals.
...
More speculatively, one can view the asymptotic-facts layer as a \emph{lemma backend}: its rules are currently justified by standard real-analysis facts, but could eventually be discharged by a separate proof kernel (for example, a minimal system based on binary lambda calculus). This would preserve the current runtime performance while strengthening the provenance of certified asymptotic inferences.
